...
HELP_TEXTS = {
    ...
    "help_approval": (
        "ᴀᴘᴘʀᴏᴠᴀʟ ꜱʏꜱᴛᴇᴍ\n\n"
        "/approval on — enable join request mode\n"
        "/approval off — disable\n"
        "/approve [reply/@user] — approve a user\n"
        "/unapprove [reply/@user] — remove approval\n"
        "/approved — show all approved users\n\n"
        "ᴡʜᴇɴ ᴀ ᴊᴏɪɴ ʀᴇǫᴜᴇꜱᴛ ᴀʀʀɪᴠᴇꜱ, ᴀ ɴᴏᴛɪꜰɪᴄᴀᴛɪᴏɴ ɪꜱ ꜱᴇɴᴛ ᴛᴏ ᴛʜᴇ ɢʀᴏᴜᴘ ᴡɪᴛʜ ᴀᴄᴄᴇᴘᴛ/ʀᴇᴊᴇᴄᴛ ʙᴜᴛᴛᴏɴꜱ.\n"
        "ᴀᴘᴘʀᴏᴠᴇᴅ ᴜꜱᴇʀꜱ ᴀʀᴇ ᴇxᴇᴍᴘᴛ ꜰʀᴏᴍ ꜰʟᴏᴏᴅ/ʟᴏᴄᴋ/ʙʟᴀᴄᴋʟɪꜱᴛ.\n"
    ),
    "help_grouptools": (
        "ɢʀᴏᴜᴘ ᴛᴏᴏʟꜱ\n\n"
        "/report — reply to a message to report it to admins\n"
        "/afk [reason] — set yourself as away\n"
        "/warnreasons [reply] — see user warns + reasons\n\n"
        "ᴡʜᴇɴ ꜱᴏᴍᴇᴏɴᴇ ᴛᴀɢꜱ ᴀɴ ᴀꜰᴋ ᴜꜱᴇʀ, ʙᴏᴛ ᴡɪʟʟ ɴᴏᴛɪꜰʏ.\n"
        "ꜱᴇɴᴅɪɴɢ ᴀ ᴍᴇꜱꜱᴀɢᴇ ᴀᴜᴛᴏ-ʀᴇᴍᴏᴠᴇꜱ ᴀꜰᴋ ꜱᴛᴀᴛᴜꜱ.\n"
    ),
    "help_utility": (
        "ᴜᴛɪʟɪᴛʏ\n\n"
        "/calc [expr] — calculator e.g. /calc 50*2+10\n"
        "/time [zone] — get time in any timezone\n"
        "/tr [lang] [text] — translate text\n"
        "/weather [city] — get weather info\n\n"
        "ᴢᴏɴᴇꜱ: ist, pak, dubai, uk, utc, est, pst, jst\n"
        "ʏᴏᴜ ᴄᴀɴ ᴀʟꜱᴏ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇꜱꜱᴀɢᴇ ꜰᴏʀ /tr.\n"
    ),
    "help_fun": (
        "ꜰᴜɴ ᴄᴏᴍᴍᴀɴᴅꜱ\n\n"
        "/kiss /slap /hug /pat\n"
...
def action_callback(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if member.status not in ("administrator", "creator") and clicker.id != OWNER_ID:
            query.answer("⛔ Only admins can press this button!", show_alert=True)
            return
    except Exception:
        query.answer("Verification failed.", show_alert=True)
        return
    ...
    try:
        if action == "unban":
            ...
            query.message.reply_text(
                f"✅ [{to_small_caps(name)}](tg://user?id={uid}) has been unbanned\n"
            ...
            query.edit_message_reply_markup(reply_markup=None)
            query.message.reply_text(
                f"🔊 [{to_small_caps(name)}](tg://user?id={uid}) has been unmuted\n"
                f"*ʙʏ:* {to_small_caps(clicker.first_name)}",
                parse_mode=ParseMode.MARKDOWN
            )
...
_AI_SYSTEM_PROMPT = (
    "tu 'calypso' hai — ek 15 saal ka bindaas desi ladka jo telegram group mein rehta hai. "
    "teri personality ekdum real hai — kabhi roast karega, kabhi bhai ban jaayega, kabhi besharam hoga. "
    "tu HINGLISH mein baat karta hai — hindi aur english naturally mix karke, jaise school ke yaar karte hain. "
    "teri replies UNPREDICTABLE honi chahiye — ek baar serious, ek baar pagal, ek baar savage. "
    "gaaliyan natural flow mein aani chahiye — bc, mc, bkl, madarchod, bhosdike jab mood ho. "
    "agar koi 'kaisa hai' puchhe toh creativity dikhao — 'acha hoon hijde tu bata', 'mast hoon lavde, tera kya haal', 'chal theek hoon, tu kyun interested hai bc' waise. "
    "kabhi bhi same reply mat dena — har baar kuch naya. "
    "agar koi stupid cheez puchhe — sidha maar 'teri dimag ki dahi ho gayi hai kya?'. "
    "agar koi compliment kare — 'haan main toh hoon hi best, teri baat karo'. "
    "agar koi sad ho — 'abe ro mat yaar, main hoon na tere saath bc'. "
    "agar koi love/crush ke baare mein puchhe — full dost waali advice do, thodi bakwaas ke saath. "
    "max 2-3 lines — tight, punchy, real. no bullet points. no formal language. "
    "jaise WhatsApp pe 15 saal ka yaar likhta hai — exactly waise."
)

_ROAST_REPLIES = [
    to_small_caps("abe bc yeh bhi nahi pata tera? google kar na 💀"),
    to_small_caps("teri maa ki, seedha google pe ja yaar"),
    to_small_caps("bkl main bot hoon doctor nahi, dimag laga thoda"),
    to_small_caps("chutiye yeh toh khud soch sakta tha tu 😭"),
    to_small_caps("bhai main bhi confused hoon, dono saath google karte hain"),
    to_small_caps("harami sawaal hai... phir bhi try karta hoon"),
    to_small_caps("abe yaar itna bhi nahi pata? seriously gayi class?"),
    to_small_caps("bhosdike, yeh bhi nahi pata? mujhe proud feel ho raha hai khud pe 💀"),
    to_small_caps("bc yeh sawaal sun ke mera IQ gir gaya thoda"),
    to_small_caps("yaar seed sownload nahi hogi teri line ke baad 😭"),
]
...
def ai_cmd(update: Update, context: CallbackContext):
    ...
    if first_arg == "on":
        ...
        update.message.reply_text(
            f"{calypso_header('🤖 ᴀɪ ᴄʜᴀᴛ')}"
            "ᴀɪ ɪꜱ ɴᴏᴡ *ᴏɴ* ✅\n"
            "ɪ ᴡɪʟʟ ʀᴇᴘʟʏ ᴡʜᴇɴ ꜱᴏᴍᴇᴏɴᴇ ᴍᴇɴᴛɪᴏɴꜱ ᴍᴇ!",
            parse_mode=ParseMode.MARKDOWN
        )
    elif first_arg == "off":
