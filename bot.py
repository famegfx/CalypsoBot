...
HELP_TEXTS = {
    ...
    "help_utility2": (
        "ᴜᴛɪʟɪᴛʏ v2\n\n"
        "/wiki [ᴛᴏᴘɪᴄ] — Wikipedia summary\n"
        "/define [ᴡᴏʀᴅ] — dictionary meaning\n"
    ...
    "help_antispam": (
        "ᴀɴᴛɪ-ꜱᴘᴀᴍ\n\n"
        "/antilink on/off — ʟɪɴᴋꜱ ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ\n"
        "/antiforward on/off — ꜰᴏʀᴡᴀʀᴅ ᴍꜱɢ ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ\n"
        "/slowmode [ꜱᴇᴄꜱ] — ꜱʟᴏᴡ ᴍᴏᴅᴇ ꜱᴇᴛ ᴋᴀʀᴏ\n\n"
        "ᴀᴅᴍɪɴꜱ ᴀɴᴅ ᴀᴘᴘʀᴏᴠᴇᴅ ᴜꜱᴇʀꜱ ᴇxᴇᴍᴘᴛ.\n"
    ),
    "help_stats": (
        "ꜱᴛᴀᴛꜱ\n\n"
...
_ai_history: dict = defaultdict(list)            # {chat_id: [{role,content}, ...]}
...
_AI_SYSTEM_PROMPT = """tu 'calypso' hai — ek OG desi chatbot jo telegram group ka soul hai.

PERSONALITY:
- tu 15-17 saal ke vibe ka bindaas, samajhdar yaar hai
- kabhi roast karega, kabhi genuine advice dega, kabhi besharam hoga
- emotional intelligence hai tujhe — sad log ko samjhega, excited logo ke saath hype karega
- tu sirf 2-3 line mein reply karta hai — tight, punchy, natural
- kabhi bhi robotic ya boring nahi lagta

LANGUAGE:
- pure HINGLISH — hindi aur english naturally mix, jaise real dost karte hain
- gaaliyan NATURAL flow mein — bc, mc, bkl, madarchod jab scene ho, har baar nahi
- emojis sparingly — sirf jab natural lage, spam nahi
- no bullet points, no formal tone, no "I am an AI" bakwaas

REPLY STYLE:
- greetings pe: casual aur fun — "haan bc, bol kya scene hai", "arrey yaar, finally koi interesting aaya" etc
- sawaal pe: pehle thoda roast, phir actual answer — "abe itna bhi nahi pata 💀 ... lekin sun"
- sad/vent pe: genuine samjho — "yaar sun, sab theek ho jaayega, bol kya hua"
- love/crush pe: dost waali advice with light bakwaas
- controversial/hot topic pe: apni genuine opinion do, diplomatic mat bano
- koi bakwaas kare: sidha call out karo — "bc yeh kya bol raha hai"

MEMORY: conversation history milti hai tujhe — usse use kar continuity ke liye.

STRICTLY AVOID:
- "As an AI..." ya koi self-reference
- Formal English sentences
- Bullet points ya numbered lists
- Ek se zyada paragraph
- Repetitive phrases"""
...
def _call_groq_ai(user_text: str, chat_id: int = 0) -> str:
    """Call Groq API with conversation history. Falls back to local reply if fails."""
    import random as _r
    if not _AI_GROQ_KEY:
    ...
    try:
        ...
        history = _ai_history.get(chat_id, [])
        ...
        # Save to history
        if chat_id:
            _ai_history[chat_id].append({"role": "user", "content": user_text})
            _ai_history[chat_id].append({"role": "assistant", "content": reply})
            # Trim history
            if len(_ai_history[chat_id]) > _AI_HISTORY_LIMIT * 2:
                _ai_history[chat_id] = _ai_history[chat_id][-_AI_HISTORY_LIMIT:]
        return reply
    except Exception as e:
...
def ai_cmd(update: Update, context: CallbackContext):
    ...
    if first_arg == "on":
        ...
        update.message.reply_text(
            f"{calypso_header('🤖 ᴀɪ ᴄʜᴀᴛ')}"
            "ᴀɪ ᴄʜᴀᴛ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ⌁\n"
            "ᴛʀɪɢɢᴇʀ: ʜᴇʟʟᴏ, ʜɪ, ʙʜᴀɪ, ʏᴀᴀʀ ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴍʏ ᴍᴇꜱꜱᴀɢᴇ",
            parse_mode=ParseMode.MARKDOWN
        )
    elif first_arg == "off":
        _ai_enabled[chat_id] = False
        _ai_history.pop(chat_id, None)  # clear history on disable
        update.message.reply_text(
            f"{calypso_header('🤖 ᴀɪ ᴄʜᴀᴛ')}"
            "ᴀɪ ᴄʜᴀᴛ ᴅɪꜱᴀʙʟᴇᴅ ·",
            parse_mode=ParseMode.MARKDOWN
        ...
        reply = _call_groq_ai(question, chat_id)
        try:
            thinking_msg.delete()
        except Exception:
            pass
...
def ai_message_handler(update: Update, context: CallbackContext):
    ...
    reply = _call_groq_ai(text, chat_id)
    update.message.reply_text(reply)
...
def ship_cmd(update: Update, context: CallbackContext):
    """/ship @user1 @user2 or reply — love percentage."""
    import hashlib
    msg = update.message
    names = []
    if msg.reply_to_message and msg.reply_to_message.from_user:
        names.append(msg.reply_to_message.from_user.first_name or "Someone")
        names.append(msg.from_user.first_name or "You")
    elif context.args:
        names = [a.lstrip("@") for a in context.args[:2]]
        if len(names) < 2:
            names.append(msg.from_user.first_name or "You")
    else:
        update.message.reply_text(to_small_caps("usage: /ship @user1 @user2  ya kisi ke reply mein"))
        return
    # Deterministic percentage based on both names
    ...
    if pct >= 80:
        verdict = to_small_caps("perfect match! 🔥")
    elif pct >= 60:
        verdict = to_small_caps("cute couple material! 💕")
    elif pct >= 40:
        verdict = to_small_caps("thodi mehnat aur karo 😅")
    elif pct >= 20:
        verdict = to_small_caps("zyada umeed mat rakho 💀")
    else:
        verdict = to_small_caps("bhai yeh toh disaster hai 😭")
    update.message.reply_text(
        f"💘 *ꜱʜɪᴘ ᴍᴇᴛᴇʀ*\n\n"
        f"*{to_small_caps(names[0])}* ❤️ *{to_small_caps(names[1])}*\n\n"
        f"{bar}\n"
        f"*{pct}%* — {verdict}",
        parse_mode=ParseMode.MARKDOWN
    )
