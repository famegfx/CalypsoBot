...
HELP_TEXTS = {
    ...
    "help_music": (
        ...
        "/stop — ꜱᴛᴏᴘ & ᴄʟᴇᴀʀ ǫᴜᴇᴜᴇ\n"
        "/np — ɴᴏᴡ ᴘʟᴀʏɪɴɢ\n"
        "/queue — ꜱʜᴏᴡ ǫᴜᴇᴜᴇ\n"
        "/loop — ᴛᴏɢɢʟᴇ ʟᴏᴏᴘ\n"
        "/shuffle — ꜱʜᴜꜰꜰʟᴇ ǫᴜᴇᴜᴇ\n"
        "/clearqueue — ᴄʟᴇᴀʀ ǫᴜᴇᴜᴇ\n"
        "/leave — ʟᴇᴀᴠᴇ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ\n\n"
        "ᴀʟʟ ᴄᴏᴍᴍᴀɴᴅꜱ ᴡᴏʀᴋ ᴡɪᴛʜ . ᴘʀᴇꜰɪx ᴛᴏᴏ.\n"
        "ʀᴇǫᴜɪʀᴇꜱ ᴀɴ ᴀᴄᴛɪᴠᴇ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ + API_ID/API_HASH/SESSION.\n"
    ),
}
...
def generate_leaderboard_image(scores: list, period_label: str) -> io.BytesIO:
    ...
    Returns: BytesIO PNG — 16:9, black bg, white sharp bars, unicode small caps
    """
    W, H = 1280, 720
    img  = Image.new("RGB", (W, H), color=(0, 0, 0))
    draw = ImageDraw.Draw(img)
    try:
        f_title  = ImageFont.truetype(_FONT_BOLD, 58)
        f_period = ImageFont.truetype(_FONT_REG,  24)
        f_name   = ImageFont.truetype(_FONT_REG,  24)
        f_count  = ImageFont.truetype(_FONT_BOLD, 21)
        f_rank   = ImageFont.truetype(_FONT_REG,  20)
        f_sig    = ImageFont.truetype(_FONT_REG,  18)
    except Exception:
        f_title = f_period = f_name = f_count = f_rank = f_sig = ImageFont.load_default()

    # Title in unicode small caps
    title = to_small_caps("leaderboard")
    tw = draw.textlength(title, font=f_title)
    draw.text(((W - tw) / 2, 20), title, fill=(255, 255, 255), font=f_title)

    # Thin underline
    draw.line([(W//2 - 180, 92), (W//2 + 180, 92)], fill=(255, 255, 255), width=1)

    # Period label in small caps
    pl = to_small_caps(period_label.lower())
    pw = draw.textlength(pl, font=f_period)
    draw.text(((W - pw) / 2, 100), pl, fill=(150, 150, 150), font=f_period)

    if not scores:
        msg = to_small_caps("no data yet — start chatting!")
        mw = draw.textlength(msg, font=f_period)
        draw.text(((W - mw) / 2, H // 2), msg, fill=(80, 80, 80), font=f_period)
    ...
    for i, (name, count) in enumerate(scores):
        ...
        draw.text((PAD_L, y + 3), f"{i+1:02}", fill=(80, 80, 80), font=f_rank)
        dname = to_small_caps((name[:18] + "\u2026") if len(name) > 18 else name)
        draw.text((PAD_L + 50, y + 3), dname, fill=(210, 210, 210), font=f_name)
        draw.rectangle([BAR_X, y, BAR_X + bar_w, y + BAR_H], fill=(255, 255, 255))
        cs = str(count)
        cw = draw.textlength(cs, font=f_count)
        if bar_w > cw + 16:
            draw.text((BAR_X + bar_w - cw - 7, y + 3), cs, fill=(0, 0, 0), font=f_count)
        else:
            draw.text((BAR_X + bar_w + 8, y + 3), cs, fill=(255, 255, 255), font=f_count)
        if i < n - 1:
            draw.line([(PAD_L, y + ROW_H - 3), (W - 60, y + ROW_H - 3)], fill=(22, 22, 22), width=1)

    sig = to_small_caps("calypso bot")
    sw = draw.textlength(sig, font=f_sig)
    draw.text((W - sw - 28, H - 30), sig, fill=(55, 55, 55), font=f_sig)
...
# ═══════════════════════════════════════════════════════
#  MUSIC SYSTEM  🎵  (pytgcalls + yt-dlp)
# ═══════════════════════════════════════════════════════
#
#  HOW IT WORKS
#  ─────────────────────────────────────────────────────
#  • Uses a Pyrogram *user-bot* account (your API_ID/API_HASH + SESSION)
#    to join the group voice chat.
#  • The python-telegram-bot (bot account) handles all /play commands.
#  • Audio is streamed via yt-dlp → ffmpeg → pytgcalls at high quality.
#
#  REQUIRED ENV VARS (in addition to BOT_TOKEN):
#    API_ID    — from https://my.telegram.org
#    API_HASH  — from https://my.telegram.org
#    SESSION   — Pyrogram string session  (generate once with gen_session.py)
#
#  INSTALL:
#    pip install py-tgcalls pyrogram yt-dlp
#    apt-get install ffmpeg   (or your OS equivalent)
#
# ═══════════════════════════════════════════════════════

# ── Per-chat music state ──────────────────────────────
class MusicState:
    def __init__(self):
        self.queue   = deque()       # list of {"title","url","req_by","thumb"}
        self.current = None          # currently playing track dict
        self.loop    = False         # loop current track
        self.paused  = False

_music_states: dict = defaultdict(MusicState)  # chat_id → MusicState

# ── Pyrogram + pytgcalls clients (initialised in main()) ──
_pyro_app   = None   # pyrogram.Client
_tgcalls    = None   # PyTgCalls

# ────────────────────────────────────────────────────────
#  Helpers
# ────────────────────────────────────────────────────────

def _yt_search(query: str) -> dict:
    """Return {title, url, thumbnail, duration} for a YouTube query/URL."""
    # If it's a direct URL, just extract info; otherwise search
    if re.match(r"https?://", query):
        cmd = ["yt-dlp", "--dump-json", "--no-playlist", query]
    else:
        cmd = ["yt-dlp", "--dump-json", "--no-playlist", f"ytsearch1:{query}"]
    try:
        ...
        logger.warning(f"yt_search error: {e}")
        return None


def _yt_audio_url(webpage_url: str) -> str:
    """Extract direct audio stream URL from a YouTube page URL."""
    cmd = [
        "yt-dlp", "-f", "bestaudio[ext=webm]/bestaudio/best",
        "--get-url", "--no-playlist", webpage_url
    ]
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
    return result.stdout.strip().splitlines()[0]


def _music_state(chat_id: int) -> MusicState:
    return _music_states[chat_id]


def _now_playing_text(track: dict) -> str:
    return (
        f"{calypso_header('🎵 ɴᴏᴡ ᴘʟᴀʏɪɴɢ')}"
        f"*{to_small_caps(track['title'][:60])}*\n"
        f"⏱ `{track.get('duration','?')}` | 👤 {track.get('req_by','?')}\n"
    )


def _queue_safe_play(chat_id: int, update, context):
    """Start playing next track in queue (runs in executor thread)."""
    if not MUSIC_ENABLED or _tgcalls is None:
        return

    state = _music_state(chat_id)
    if not state.queue and not state.loop:
        state.current = None
        return

    track = state.current if (state.loop and state.current) else state.queue.popleft()
    state.current = track

    try:
        audio_url = _yt_audio_url(track["url"])
        stream    = AudioPiped(audio_url, HighQualityAudio())
        loop      = asyncio.new_event_loop()
        loop.run_until_complete(_tgcalls.change_stream(chat_id, stream))
        loop.close()
    except NotInGroupCallError:
        # Bot not in call — join first, then play
        try:
            audio_url = _yt_audio_url(track["url"])
            stream    = AudioPiped(audio_url, HighQualityAudio())
            loop      = asyncio.new_event_loop()
            loop.run_until_complete(_tgcalls.join_group_call(chat_id, stream))
            loop.close()
        except Exception as e:
            if update:
                update.message.reply_text(f"❌ ᴄᴏᴜʟᴅ ɴᴏᴛ ᴊᴏɪɴ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ: `{e}`",
                                          parse_mode=ParseMode.MARKDOWN)
            return
    except Exception as e:
        logger.warning(f"_queue_safe_play error: {e}")
        return

    if update and update.message:
        update.message.reply_text(
            _now_playing_text(track),
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True
        )
...
def music_play(update: Update, context: CallbackContext):
    """/play <song name or YouTube URL>"""
    chat = update.effective_chat
    if chat.type == "private":
        update.message.reply_text("⚠️ ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ɪɴ ᴀ ɢʀᴏᴜᴘ ᴡɪᴛʜ ᴀɴ ᴀᴄᴛɪᴠᴇ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ.")
        return
    if not MUSIC_ENABLED:
        update.message.reply_text("❌ ᴍᴜꜱɪᴄ ᴍᴏᴅᴜʟᴇ ɴᴏᴛ ɪɴꜱᴛᴀʟʟᴇᴅ.\n`pip install py-tgcalls pyrogram yt-dlp`",
                                  parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if not query:
        update.message.reply_text(
            f"{calypso_header('🎵 ᴍᴜꜱɪᴄ')}"
            "ᴜꜱᴀɢᴇ: `/play <ꜱᴏɴɢ ɴᴀᴍᴇ ᴏʀ ᴜʀʟ>`",
            parse_mode=ParseMode.MARKDOWN
        )
        return

    msg = update.message.reply_text("🔍 ꜱᴇᴀʀᴄʜɪɴɢ...")

    def _do():
        track = _yt_search(query)
        if not track:
            msg.edit_text("❌ ꜱᴏɴɢ ɴᴏᴛ ꜰᴏᴜɴᴅ. ᴛʀʏ ᴀɴᴏᴛʜᴇʀ ᴋᴇʏᴡᴏʀᴅ.")
            return

        track["req_by"] = update.effective_user.first_name
        state  = _music_state(chat.id)
        state.queue.append(track)

        if state.current is None:
            msg.edit_text("⏳ ʟᴏᴀᴅɪɴɢ ᴀᴜᴅɪᴏ...")
            _queue_safe_play(chat.id, update, context)
            msg.delete()
        else:
            pos = len(state.queue)
            msg.edit_text(
                f"{calypso_header('➕ ᴀᴅᴅᴇᴅ ᴛᴏ ǫᴜᴇᴜᴇ')}"
                f"*{to_small_caps(track['title'][:60])}*\n"
                f"⏱ `{track.get('duration','?')}` | ᴘᴏꜱɪᴛɪᴏɴ *#{pos}*",
                parse_mode=ParseMode.MARKDOWN
            )
...
def music_skip(update: Update, context: CallbackContext):
    ...
    state = _music_state(chat.id)
    if state.current is None:
        update.message.reply_text("⚠️ ɴᴏᴛʜɪɴɢ ɪꜱ ᴘʟᴀʏɪɴɢ ʀɪɢʜᴛ ɴᴏᴡ.")
        return
    ...
    def _do():
        try:
            if state.queue:
                ...
                loop = asyncio.new_event_loop()
                loop.run_until_complete(_tgcalls.leave_group_call(chat.id))
                loop.close()
                state.current = None
                update.message.reply_text("⏹ ǫᴜᴇᴜᴇ ꜰɪɴɪꜱʜᴇᴅ. ʟᴇꜰᴛ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ.")
        except Exception as e:
            update.message.reply_text(f"❌ ꜱᴋɪᴘ ꜰᴀɪʟᴇᴅ: `{e}`", parse_mode=ParseMode.MARKDOWN)

    update.message.reply_text("⏭ ꜱᴋɪᴘᴘᴇᴅ.")
    threading.Thread(target=_do, daemon=True).start()


def music_pause(update: Update, context: CallbackContext):
    ...
    state = _music_state(chat.id)
    if state.current is None:
        update.message.reply_text("⚠️ ɴᴏᴛʜɪɴɢ ɪꜱ ᴘʟᴀʏɪɴɢ.")
        return
    if state.paused:
        update.message.reply_text("⚠️ ᴀʟʀᴇᴀᴅʏ ᴘᴀᴜꜱᴇᴅ. ᴜꜱᴇ /ʀᴇꜱᴜᴍᴇ.")
        return

    def _do():
        try:
            loop = asyncio.new_event_loop()
            loop.run_until_complete(_tgcalls.pause_stream(chat.id))
            loop.close()
            state.paused = True
            update.message.reply_text("⏸ ᴘᴀᴜꜱᴇᴅ. ᴜꜱᴇ /ʀᴇꜱᴜᴍᴇ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ.")
        except Exception as e:
            update.message.reply_text(f"❌ `{e}`", parse_mode=ParseMode.MARKDOWN)

    threading.Thread(target=_do, daemon=True).start()


def music_resume(update: Update, context: CallbackContext):
    ...
    state = _music_state(chat.id)
    if not state.paused:
        update.message.reply_text("⚠️ ᴘʟᴀʏʙᴀᴄᴋ ɪꜱ ɴᴏᴛ ᴘᴀᴜꜱᴇᴅ.")
        return

    def _do():
        try:
            loop = asyncio.new_event_loop()
            loop.run_until_complete(_tgcalls.resume_stream(chat.id))
            loop.close()
            state.paused = False
            update.message.reply_text("▶️ ʀᴇꜱᴜᴍᴇᴅ.")
        except Exception as e:
            update.message.reply_text(f"❌ `{e}`", parse_mode=ParseMode.MARKDOWN)
...
def music_stop(update: Update, context: CallbackContext):
    ...
    def _do():
        try:
            ...
            pass
        update.message.reply_text(
            f"{calypso_header('⏹ ᴍᴜꜱɪᴄ ꜱᴛᴏᴘᴘᴇᴅ')}"
            "ǫᴜᴇᴜᴇ ᴄʟᴇᴀʀᴇᴅ. ʟᴇꜰᴛ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ.",
            parse_mode=ParseMode.MARKDOWN
        )
...
def music_queue(update: Update, context: CallbackContext):
    """/queue — show current queue"""
    chat  = update.effective_chat
    state = _music_state(chat.id)

    lines = [calypso_header("🎶 ᴍᴜꜱɪᴄ ǫᴜᴇᴜᴇ")]

    if state.current:
        lines.append(f"▶️ *{to_small_caps(state.current['title'][:50])}* `[{state.current.get('duration','?')}]`")
    else:
        lines.append(f"▶️ _{to_small_caps('nothing playing')}_")

    if state.queue:
        lines.append("")
        for i, t in enumerate(list(state.queue)[:10], 1):
            lines.append(f"`{i}.` {to_small_caps(t['title'][:45])} `[{t.get('duration','?')}]`")
        if len(state.queue) > 10:
            lines.append(f"_... ᴀɴᴅ {len(state.queue)-10} ᴍᴏʀᴇ_")
    else:
        lines.append(f"\n_{to_small_caps('queue is empty')}_")

    loop_icon = "🔂" if state.loop else "➡️"
    lines.append(f"\n{loop_icon} ʟᴏᴏᴘ: `{'ᴏɴ' if state.loop else 'ᴏꜰꜰ'}`")

    update.message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN,
                               disable_web_page_preview=True)


def music_nowplaying(update: Update, context: CallbackContext):
    """/np — show now playing"""
    state = _music_state(update.effective_chat.id)
    if state.current:
        update.message.reply_text(
            _now_playing_text(state.current),
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True
        )
    else:
        update.message.reply_text(f"⚠️ {to_small_caps('nothing is playing right now.')}")


def music_loop(update: Update, context: CallbackContext):
    """/loop — toggle loop for current track"""
    state = _music_state(update.effective_chat.id)
    state.loop = not state.loop
    status = "🔂 ʟᴏᴏᴘ *ᴇɴᴀʙʟᴇᴅ*." if state.loop else "➡️ ʟᴏᴏᴘ *ᴅɪꜱᴀʙʟᴇᴅ*."
    update.message.reply_text(status, parse_mode=ParseMode.MARKDOWN)


def music_shuffle(update: Update, context: CallbackContext):
    """/shuffle — shuffle the queue"""
    import random
    state = _music_state(update.effective_chat.id)
    if len(state.queue) < 2:
        update.message.reply_text("⚠️ ɴᴏᴛ ᴇɴᴏᴜɢʜ ᴛʀᴀᴄᴋꜱ ᴛᴏ ꜱʜᴜꜰꜰʟᴇ.")
        return
    q_list = list(state.queue)
    random.shuffle(q_list)
    state.queue = deque(q_list)
    update.message.reply_text("🔀 ǫᴜᴇᴜᴇ ꜱʜᴜꜰꜰʟᴇᴅ.")


def music_clearqueue(update: Update, context: CallbackContext):
    """/clearqueue — clear the queue (keep current)"""
    state = _music_state(update.effective_chat.id)
    state.queue.clear()
    update.message.reply_text("🗑 ǫᴜᴇᴜᴇ ᴄʟᴇᴀʀᴇᴅ.")


def music_leave(update: Update, context: CallbackContext):
    """/leave — force leave voice chat"""
    if not MUSIC_ENABLED or _tgcalls is None:
        return
    chat  = update.effective_chat
    state = _music_state(chat.id)
    ...
    def _do():
        try:
            ...
            loop.run_until_complete(_tgcalls.leave_group_call(chat.id))
            loop.close()
            update.message.reply_text("👋 ʟᴇꜰᴛ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ.")
        except Exception as e:
            update.message.reply_text(f"❌ `{e}`", parse_mode=ParseMode.MARKDOWN)
...
def music_help(update: Update, context: CallbackContext):
    """/music — show all music commands"""
    update.message.reply_text(
        f"{calypso_header('🎵 ᴍᴜꜱɪᴄ ᴄᴏᴍᴍᴀɴᴅꜱ')}"
        "/play `<ꜱᴏɴɢ/ᴜʀʟ>` — ᴘʟᴀʏ ꜱᴏɴɢ ɪɴ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ\n"
        "/skip — ꜱᴋɪᴘ ᴛᴏ ɴᴇxᴛ ꜱᴏɴɢ\n"
        "/pause — ᴘᴀᴜꜱᴇ ᴘʟᴀʏʙᴀᴄᴋ\n"
        "/resume — ʀᴇꜱᴜᴍᴇ ᴘʟᴀʏʙᴀᴄᴋ\n"
        "/stop — ꜱᴛᴏᴘ ᴀɴᴅ ᴄʟᴇᴀʀ ǫᴜᴇᴜᴇ\n"
        "/np — ɴᴏᴡ ᴘʟᴀʏɪɴɢ\n"
        "/queue — ꜱʜᴏᴡ ǫᴜᴇᴜᴇ\n"
        "/loop — ᴛᴏɢɢʟᴇ ʟᴏᴏᴘ\n"
        "/shuffle — ꜱʜᴜꜰꜰʟᴇ ǫᴜᴇᴜᴇ\n"
        "/clearqueue — ᴄʟᴇᴀʀ ǫᴜᴇᴜᴇ\n"
        "/leave — ʟᴇᴀᴠᴇ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ\n\n"
        "ᴀʟꜱᴏ ᴡᴏʀᴋꜱ ᴡɪᴛʜ `.` ᴘʀᴇꜰɪx.\n"
        "_ʀᴇǫᴜɪʀᴇꜱ: ᴀɴ ᴀᴄᴛɪᴠᴇ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ._",
        parse_mode=ParseMode.MARKDOWN
    )
...
_DOT_COMMANDS = {
    ...
    # ── Music ──────────────────────────────────────────
    "play":         lambda u, c: music_play(u, c),
    "skip":         lambda u, c: music_skip(u, c),
    "pause":        lambda u, c: music_pause(u, c),
    "resume":       lambda u, c: music_resume(u, c),
    "stop":         lambda u, c: music_stop(u, c),
    "np":           lambda u, c: music_nowplaying(u, c),
    "queue":        lambda u, c: music_queue(u, c),
    "loop":         lambda u, c: music_loop(u, c),
    "shuffle":      lambda u, c: music_shuffle(u, c),
    "clearqueue":   lambda u, c: music_clearqueue(u, c),
    "leave":        lambda u, c: music_leave(u, c),
    "music":        lambda u, c: music_help(u, c),
}
...
def main():
    ...
    # ── Music commands ────────────────────────────────
    dp.add_handler(CommandHandler("play",       music_play))
    dp.add_handler(CommandHandler("skip",       music_skip))
    dp.add_handler(CommandHandler("pause",      music_pause))
    dp.add_handler(CommandHandler("resume",     music_resume))
    dp.add_handler(CommandHandler("stop",       music_stop))
    dp.add_handler(CommandHandler("np",         music_nowplaying))
    dp.add_handler(CommandHandler("queue",      music_queue))
    dp.add_handler(CommandHandler("loop",       music_loop))
    dp.add_handler(CommandHandler("shuffle",    music_shuffle))
    dp.add_handler(CommandHandler("clearqueue", music_clearqueue))
    dp.add_handler(CommandHandler("leave",      music_leave))
    dp.add_handler(CommandHandler("music",      music_help))
    ...
    # ── Boot pytgcalls music engine ───────────────────────
    if MUSIC_ENABLED and API_ID and API_HASH and SESSION:
        global _pyro_app, _tgcalls
        _pyro_app = Client(
            "calypso_music",
            api_id=API_ID,
            api_hash=API_HASH,
        ...
        _tgcalls = PyTgCalls(_pyro_app)

        @_tgcalls.on_stream_end()
        async def _on_stream_end(client, update):
            chat_id = update.chat_id
            state   = _music_state(chat_id)
            if state.loop and state.current:
                # replay same track
                audio_url = _yt_audio_url(state.current["url"])
                await client.change_stream(chat_id, AudioPiped(audio_url, HighQualityAudio()))
            elif state.queue:
        ...
        def _start_tgcalls():
            ...
            loop.run_until_complete(_pyro_app.start())
            loop.run_until_complete(_tgcalls.start())
            loop.run_forever()

        threading.Thread(target=_start_tgcalls, daemon=True).start()
        logger.info("🎵 ᴍᴜꜱɪᴄ ᴇɴɢɪɴᴇ (ᴘʏᴛɢᴄᴀʟʟꜱ) ꜱᴛᴀʀᴛᴇᴅ.")
    else:
        if not MUSIC_ENABLED:
            logger.warning("🎵 ᴍᴜꜱɪᴄ ᴅɪꜱᴀʙʟᴇᴅ — ɪɴꜱᴛᴀʟʟ ᴘʏᴛɢᴄᴀʟʟꜱ + ᴘʏʀᴏɢʀᴀᴍ.")
        else:
            logger.warning("🎵 ᴍᴜꜱɪᴄ ᴅɪꜱᴀʙʟᴇᴅ — ꜱᴇᴛ ᴀᴘɪ_ɪᴅ, ᴀᴘɪ_ʜᴀꜱʜ, ꜱᴇꜱꜱɪᴏɴ ᴇɴᴠ ᴠᴀʀꜱ.")
