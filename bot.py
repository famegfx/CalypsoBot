...
HELP_TEXTS = {
    ...
    "help_joinreq": (
        "🚪 ᴊᴏɪɴ ʀᴇǫᴜᴇꜱᴛ\n\n"
        "/joinreq on — ᴇɴᴀʙʟᴇ ᴋᴀʀᴏ\n"
        "/joinreq off — ᴅɪꜱᴀʙʟᴇ ᴋᴀʀᴏ\n\n"
        "ʜᴀʀ ʀᴇǫᴜᴇꜱᴛ ᴀᴅᴍɪɴꜱ ᴋᴇ PM ᴍᴇɪɴ ᴀᴀʏᴇɢɪ\n"
        "Approve/Reject ʙᴜᴛᴛᴏɴ ꜱᴇ ᴀᴄᴛɪᴏɴ ʟᴇᴏ.\n"
    ),
    "help_fun": (
        "🎉 ꜰᴜɴ ᴄᴏᴍᴍᴀɴᴅꜱ\n\n"
        "/kiss /slap /hug /pat\n"
...
_AI_GROQ_KEY = os.environ.get("GROQ_API_KEY", "")   # Optional: set for real AI
_ai_enabled: dict = defaultdict(lambda: False)       # {chat_id: bool}

_AI_SYSTEM_PROMPT = (
    "tu 'calypso' hai — ek savage, witty telegram group bot. "
    "tu hinglish mein baat karta hai — matlab thoda hindi, thoda english mix. "
    "teri replies short, roasted, aur funny honi chahiye. "
    "tu information bhi de sakta hai lekin attitude ke saath. "
    "kabhi bhi boring mat ho. small caps unicode use karo replies mein. "
    "agar koi stupid question kare toh gently roast karo. "
    "max 3-4 lines mein reply karo, never long paragraphs."
)

_ROAST_REPLIES = [
    to_small_caps("bhai serious? yeh bhi khud se nahi sooch saka?"),
    to_small_caps("arey waah, kya sawaal hai... bilkul bekaar 💀"),
    to_small_caps("google toh hai na tere paas bhai?"),
    to_small_caps("yeh poochh ke apna time waste kiya tune"),
    to_small_caps("main bot hoon, pagal nahi — thoda dimag laga"),
    to_small_caps("iss sawaal ka jawab dene ke liye main nahi bana"),
    to_small_caps("beta, pehle khud soch, phir puchh"),
]
...
def ai_cmd(update: Update, context: CallbackContext):
    ...
    if first_arg == "on":
        ...
        update.message.reply_text(to_small_caps("sooch raha hoon..."))
        reply = _call_groq_ai(question)
        update.message.reply_text(to_small_caps(reply) if reply.isascii() else reply)
...
# ═══════════════════════════════════════════════════════
#  21. JOIN REQUEST SYSTEM  🚪
# ═══════════════════════════════════════════════════════
# /joinreq on  — enable join request mode (admin approves/rejects)
# /joinreq off — disable
# When a join request comes in, admins get a PM with Approve/Reject buttons
# ═══════════════════════════════════════════════════════

_joinreq_enabled: dict = defaultdict(lambda: False)   # {chat_id: bool}


@admin_required
def joinreq_cmd(update: Update, context: CallbackContext):
    """/joinreq on|off — toggle join request approval system."""
    chat_id = update.effective_chat.id
    if not context.args:
        status = "ᴏɴ ✅" if _joinreq_enabled[chat_id] else "ᴏꜰꜰ ❌"
        update.message.reply_text(
            f"{calypso_header('🚪 ᴊᴏɪɴ ʀᴇǫᴜᴇꜱᴛ')}"
            f"ꜱᴛᴀᴛᴜꜱ: *{status}*\n"
            "`/joinreq on` — ᴇɴᴀʙʟᴇ\n"
            "`/joinreq off` — ᴅɪꜱᴀʙʟᴇ",
            parse_mode=ParseMode.MARKDOWN
    ...
    val = context.args[0].lower()
    if val == "on":
        _joinreq_enabled[chat_id] = True
        update.message.reply_text(
            "✅ ᴊᴏɪɴ ʀᴇǫᴜᴇꜱᴛ ꜱʏꜱᴛᴇᴍ *ᴏɴ* ᴋᴀʀ ᴅɪʏᴀ.\n"
            + to_small_caps("ab har join request admins ke paas aayegi."),
            parse_mode=ParseMode.MARKDOWN
        )
    elif val == "off":
        _joinreq_enabled[chat_id] = False
        update.message.reply_text("❌ ᴊᴏɪɴ ʀᴇǫᴜᴇꜱᴛ ꜱʏꜱᴛᴇᴍ ᴏꜰꜰ.")
    else:
        update.message.reply_text("ᴜꜱᴀɢᴇ: /joinreq on|off")


def handle_join_request(update: Update, context: CallbackContext):
    """Handle incoming join requests — notify admins with approve/reject buttons."""
    req = update.chat_join_request
    if not req:
        return
    chat_id = req.chat.id
    if not _joinreq_enabled.get(chat_id):
    ...
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ ᴀᴘᴘʀᴏᴠᴇ", callback_data=f"jr_approve_{chat_id}_{user.id}"),
        InlineKeyboardButton("❌ ʀᴇᴊᴇᴄᴛ",  callback_data=f"jr_reject_{chat_id}_{user.id}"),
    ]])
    # Notify all admins in their PM
    try:
        admins = req.chat.get_administrators()
    except Exception:
        admins = []
    msg_text = (
        f"{calypso_header('🚪 ᴊᴏɪɴ ʀᴇǫᴜᴇꜱᴛ')}"
        f"*ɢʀᴏᴜᴘ:* {req.chat.title}\n"
        f"*ᴜꜱᴇʀ:* [{to_small_caps(user.first_name)}](tg://user?id={user.id})\n"
        f"*ᴜꜱᴇʀɴᴀᴍᴇ:* {uname}\n"
        f"*ɪᴅ:* `{user.id}`"
    )
    for admin in admins:
...
def join_request_callback(update: Update, context: CallbackContext):
    """Approve or reject a join request via inline button."""
    query = update.callback_query
    query.answer()
    data = query.data
    try:
    ...
    try:
        if action == "approve":
            ...
            query.edit_message_text(
                query.message.text + f"\n\n✅ *{to_small_caps('approved')}* ʙʏ {to_small_caps(query.from_user.first_name)}",
                parse_mode=ParseMode.MARKDOWN
            )
        else:
            context.bot.decline_chat_join_request(chat_id, user_id)
            query.edit_message_text(
                query.message.text + f"\n\n❌ *{to_small_caps('rejected')}* ʙʏ {to_small_caps(query.from_user.first_name)}",
                parse_mode=ParseMode.MARKDOWN
            )
    except Exception as e:
...
# GIF URLs from tenor (public embed links)
_FUN_GIFS = {
    "kiss": [
        "https://media.tenor.com/3OxLPPknmjkAAAAC/kiss.gif",
        "https://media.tenor.com/YkD-2RqRNAIAAAAC/anime-kiss.gif",
        "https://media.tenor.com/9KIF1GHjmDcAAAAC/kiss-cute.gif",
    ],
    "slap": [
        "https://media.tenor.com/m5s9mEfHkhMAAAAC/anime-slap.gif",
        "https://media.tenor.com/pjbKVW2gR0UAAAAC/slap-anime.gif",
        "https://media.tenor.com/kL5MaRDp5ZYAAAAC/slap.gif",
    ],
    "hug": [
        "https://media.tenor.com/b17OslX8wOQAAAAC/hug-anime.gif",
        "https://media.tenor.com/OHFXMHpvFU4AAAAC/anime-hug.gif",
        "https://media.tenor.com/uXhqaRxhLqkAAAAC/hug-cute.gif",
    ],
    "pat": [
        "https://media.tenor.com/N6B2EmR0TYYAAAAC/pat-anime.gif",
        "https://media.tenor.com/V1P5J0HOvwMAAAAC/anime-head-pat.gif",
        "https://media.tenor.com/MBcm5pO4CCIAAAAC/pat-headpat.gif",
    ],
    "punch": [
        "https://media.tenor.com/pJCQPbz7IG0AAAAC/anime-punch.gif",
        "https://media.tenor.com/4wN4VMXbDz8AAAAC/punch-anime.gif",
        "https://media.tenor.com/X4p_7E_xBW4AAAAC/punch.gif",
    ],
    "cuddle": [
        "https://media.tenor.com/Q0OaKAzNPlIAAAAC/cuddle-anime.gif",
        "https://media.tenor.com/mHBqvPpIVL0AAAAC/cuddle.gif",
        "https://media.tenor.com/qBLFDI2X6f4AAAAC/cuddle-cute.gif",
    ],
    "poke": [
        "https://media.tenor.com/2L_pV7sB0EMAAAAC/poke-anime.gif",
        "https://media.tenor.com/XSijwOPXqq4AAAAC/poke.gif",
        "https://media.tenor.com/AJfQNBlg5EAAAAAC/anime-poke.gif",
    ],
    "highfive": [
        "https://media.tenor.com/sX4IOmJC0oUAAAAC/high-five-anime.gif",
        "https://media.tenor.com/VBqPjwf3WIIAAAAC/anime-high-five.gif",
        "https://media.tenor.com/r6Pu2sQXhHYAAAAC/high-five.gif",
    ],
}
...
def _make_fun_handler(action: str):
    def handler(update: Update, context: CallbackContext):
        ...
        gif_url = _random.choice(_FUN_GIFS[action])
        ...
        try:
            ...
            msg.reply_text(f"{'💋' if action=='kiss' else '👊'} {caption}")
    return handler
...
_DOT_COMMANDS = {
    ...
    "joinreq":      lambda u, c: joinreq_cmd(u, c),
...
def main():
    ...
    dp.add_handler(CommandHandler("joinreq",  joinreq_cmd))
