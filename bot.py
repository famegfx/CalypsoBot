...
HELP_TEXTS = {
    ...
    "help_locks": (
        ...
        "ᴛʏᴘᴇꜱ: all, media, sticker, gif, url, forward, photo, video, voice, document\n"
    ),
    "help_filters": (
        "ꜰɪʟᴛᴇʀꜱ\n\n"
...
def resetwarns(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        f"✅ ᴡᴀʀɴꜱ ʀᴇꜱᴇᴛ ꜰᴏʀ [{name}](tg://user?id={uid}).",
        parse_mode=ParseMode.MARKDOWN
    )
...
def toggle_welcome(update: Update, context: CallbackContext):
    ...
    if args and args[0].lower() in ("on", "off"):
        ...
        update.message.reply_text(f"ᴜꜱᴀɢᴇ: /{key} on/off")
...
def lock(update: Update, context: CallbackContext):
    ...
    if ltype == "all":
        ...
        update.message.reply_text(f"❌ ᴜɴᴋɴᴏᴡɴ ᴛʏᴘᴇ. ᴀᴠᴀɪʟᴀʙʟᴇ: {', '.join(LOCK_TYPES.keys())}")
...
def promote(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.message.reply_text(
            ...
            f"[{name}](tg://user?id={uid}) ɪꜱ ɴᴏᴡ ᴀɴ ᴀᴅᴍɪɴ!",
            parse_mode=ParseMode.MARKDOWN
        )
    except BadRequest as e:
        update.message.reply_text(f"❌ ꜰᴀɪʟᴇᴅ: {e.message}")
...
def captcha_cmd(update: Update, context: CallbackContext):
    ...
    if context.args and context.args[0].lower() in ("on", "off"):
        ...
        update.message.reply_text("ᴜꜱᴀɢᴇ: /captcha on/off")
...
if __name__ == "__main__":
    ...
    class KeepAliveHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b"ᴄᴀʟʏᴘꜱᴏʙᴏᴛ ɪꜱ ᴀʟɪᴠᴇ!")
